...
class TestGetCancellationPolicy(unittest.TestCase):
    ...
    def test_raises_exception_when_datetime_now_equals_to_booking_start(self, datetime_mock):
        datetime_mock.now.return_value = datetime(year=2020, month=4, day=20) + timedelta(hours=1)
        conditions = [{'percent': 50}]
        booking_start = datetime(year=2020, month=4, day=20) + timedelta(hours=1)
        with self.assertRaises(Exception):
            get_cancellation_policy(conditions, 10, booking_start)

    @patch('cancellation_policy.datetime')
    def test_raises_exception_when_datetime_now_is_greater_than_booking_start(self, datetime_mock):
        datetime_mock.now.return_value = datetime(year=2020, month=4, day=22) + timedelta(hours=1)
        conditions = [{'percent': 50}]
        booking_start = datetime(year=2020, month=4, day=20) + timedelta(hours=1)
        with self.assertRaises(Exception):
            get_cancellation_policy(conditions, 10, booking_start)
    ...
    def test_cancellation_fee_with_only_one_condition(self, datetime_mock):
        ...
        conditions = [{'percent': 50}]
        price = 100
        booking_start = datetime(year=2020, month=4, day=21) + timedelta(hours=14)
    ...
    def test_cancellation_fee_with_several_conditions_and_now_in_them(self, datetime_mock):
        ...
        price = 100
        booking_start = datetime(year=2020, month=4, day=20) + timedelta(hours=11)
